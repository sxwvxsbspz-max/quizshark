...
"""
ElevenLabs v3 Quizmaster TTS (HaveIEver)
- liest haveiever/questions.json
- erzeugt ZWEI MP3s pro Frage in ./media/audio
  - poll_text  -> pre_audio_<id>.mp3  -> JSON-Feld: pre_audio
  - mc_text    -> question_<id>.mp3   -> JSON-Feld: audio
- v3 Handling:
...
- Protokoll:
  ...
  - schreibt in haveiever/11labsprotokoll.txt

Datei:
  haveiever/elevenlabsv3.py
"""
...
BASE_DIR = os.path.dirname(os.path.abspath(__file__))           # .../haveiever
...
def main():
    ...
    append_log("")
    append_log("============================================================")
    append_log(f"[{now_stamp()}] START elevenlabsv3.py (HaveIEver)")
    ...
    print("🎙️ Generiere HaveIEver-Audio (Leon Stern · v3 Quizmaster)")
    ...
    for idx, q in enumerate(questions, start=1):
        ...
        # -------------------------------------------------
        # POLL AUDIO (pre_audio)
        # -------------------------------------------------

        poll_text = (q.get("poll_text") or "").strip()
        pre_audio_name = f"pre_audio_{qid}.mp3"
        pre_audio_path = os.path.join(AUDIO_DIR, pre_audio_name)

        if poll_text:
            if (q.get("pre_audio") or "").strip() != pre_audio_name:
                q["pre_audio"] = pre_audio_name
                atomic_write(QUESTIONS_JSON, questions)
                updated_json += 1
                append_log(f"[JSON] idx={idx} id={qid} pre_audio gesetzt -> {pre_audio_name}")

            if (not FORCE_REGENERATE) and os.path.exists(pre_audio_path):
                skipped += 1
                append_log(f"[SKIP] idx={idx} id={qid} -> existiert bereits ({pre_audio_name})")
            else:
                print(f"[{idx}] {pre_audio_name}")
                append_log(f"[GEN ] idx={idx} id={qid} -> {pre_audio_name}")
                try:
                    generate_audio(poll_text, pre_audio_path)
                    produced += 1
                    append_log(f"[OK  ] idx={idx} id={qid} -> erzeugt ({pre_audio_name})")
                    time.sleep(0.35)
            ...
            skipped += 1
            append_log(f"[SKIP] idx={idx} id={qid} -> poll_text leer")

        # -------------------------------------------------
        # MC AUDIO (audio)
        # -------------------------------------------------

        mc_text = (q.get("mc_text") or "").strip()
        audio_name = f"question_{qid}.mp3"
        ...
        if mc_text:
            ...
            if (not FORCE_REGENERATE) and os.path.exists(audio_path):
                ...
                try:
                    generate_audio(mc_text, audio_path)
                    produced += 1
                    append_log(f"[OK  ] idx={idx} id={qid} -> erzeugt ({audio_name})")
                    time.sleep(0.35)
                except Exception as e:
                    failed += 1
                    append_log(f"[ERR ] idx={idx} id={qid} -> {e}")
            ...
            skipped += 1
            append_log(f"[SKIP] idx={idx} id={qid} -> mc_text leer")
