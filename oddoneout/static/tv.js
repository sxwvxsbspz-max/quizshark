...
/* ---------- Background Music ---------- */
const bgm = new Audio('/oddoneout/media/gamesounds/background.mp3');
bgm.preload = 'auto';
bgm.loop = true;
bgm.volume = 1.0;
...
/* ---------- Game SFX ---------- */
const SFX_BASE = '/oddoneout/media/gamesounds';
...
function setQuestionImage(imageFile) {
  const fallback = '/oddoneout/media/dummy.svg';
...
function playQuestionAudio(audioFile) {
  ...
  questionAudio.src = `/oddoneout/media/audio/${audioFile}`;
  questionAudio.load();
...
socket.on('play_round_video', data => {
  ...
  clearUnveil();
  clearAllScorePops();
  showVideo(`/oddoneout/media/frage${data.round}.mp4`);
});
