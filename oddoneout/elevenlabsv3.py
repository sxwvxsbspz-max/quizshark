...
- Standard-Naming für neue Dateien: <id>.mp3
- vorhandene audio-Einträge im JSON werden respektiert:
  - wenn audio gesetzt ist und Datei existiert -> skip
  - wenn audio gesetzt ist und Datei fehlt -> neu erzeugen mit genau diesem Namen
  - wenn audio leer ist -> neue Datei <id>.mp3 erzeugen und ins JSON schreiben
- v3 Handling:
...
def main():
    ...
    for idx, q in enumerate(questions, start=1):
        ...
        existing_audio = (q.get("audio") or "").strip()

        # -------------------------------------------------
        # FALL 1: audio ist gesetzt -> vorhandenen Namen nutzen
        # -------------------------------------------------
        if existing_audio:
            audio_name = existing_audio

            # Optional absichern, falls nur ein Name ohne Endung drinsteht
            if "." not in os.path.basename(audio_name):
                audio_name += ".mp3"
            ...
            if (not FORCE_REGENERATE) and os.path.exists(out_path):
                skipped += 1
                append_log(f"[SKIP] idx={idx} id={qid} -> audio existiert bereits ({audio_name})")
                continue

            append_log(f"[MISS] idx={idx} id={qid} -> audio fehlt, wird neu erzeugt ({audio_name})")

        # -------------------------------------------------
        # FALL 2: audio ist leer -> neuen Standardnamen setzen
        # -------------------------------------------------
        else:
            ...
            if (not FORCE_REGENERATE) and os.path.exists(out_path):
                skipped += 1
                append_log(f"[SKIP] idx={idx} id={qid} -> Standarddatei existiert bereits ({audio_name})")
                continue
