...
class PunktesammlerQuestionSource:
    ...
    def _append_cleanup_log(self, *, q: dict, provider: str, reason: str) -> None:
        """
        Protokolliert Löschungen in ./soundyear/audio_cleanup_log.txt
        """
        try:
            lines = []
            ...
            lines.append(str(q.get("year") if q.get("year") not in ("", None) else ""))
            lines.append("")
            lines.append("WRONG:")
            # Soundyear ist Freitext, hat i.d.R. keine wrong[] – wir loggen trotzdem, falls vorhanden
            for w in (q.get("wrong") or []):
                lines.append(f"- {w}")
            if not (q.get("wrong") or []):
