...
// Freitext Year UI
const yearBox   = document.getElementById('year-answer-box');
const yearValue = document.getElementById('year-answer-value');
...
/* ---------- Background Music ---------- */
const bgm = new Audio('/soundyear/media/gamesounds/background.mp3');
bgm.preload = 'auto';
bgm.loop = true;
bgm.volume = 1.0;
...
/* ---------- Game SFX ---------- */
const SFX_BASE = '/soundyear/media/gamesounds';
...
function hideYearBox() {
  if (yearBox) yearBox.classList.add('is-hidden');
  if (yearValue) yearValue.textContent = '—';
}

function showYearBox(text) {
  if (yearBox) yearBox.classList.remove('is-hidden');
  if (yearValue) yearValue.textContent = (text ?? '—');
}
...
function resetPlayerClassesForNewQuestion() {
  ...
  sidebar.querySelectorAll('.ps__player').forEach(row => {
    row.classList.remove(
      ...
      'is-correct', 'is-wrong',
      // NEU: Distance-Heatmap Klassen wie im Controller
      'is-d0','is-d1','is-d2','is-d3','is-d4','is-d5'
    );
...
/* =========================================================
   NEU: Distance-Heatmap wie im Controller (pro Spieler im TV)
   - setzt .is-d0..is-d5 auf .ps__player (Row)
   ========================================================= */

function clearPlayerDistanceClasses(row) {
  if (!row) return;
  row.classList.remove('is-d0','is-d1','is-d2','is-d3','is-d4','is-d5');
}

function applyPlayerDistanceClass(playerId, diff) {
  const row = getPlayerRow(playerId);
  if (!row) return;

  clearPlayerDistanceClasses(row);

  if (diff === null || diff === undefined || !Number.isFinite(diff)) return;

  const d = Math.abs(Number(diff));
  if (d === 0) return row.classList.add('is-d0');
  if (d === 1) return row.classList.add('is-d1');
  if (d === 2) return row.classList.add('is-d2');
  if (d === 3) return row.classList.add('is-d3');
  if (d === 4) return row.classList.add('is-d4');
  return row.classList.add('is-d5'); // >4
}


/* ---------- IMAGE / DUMMY ---------- */
function setQuestionImage(imageFile) {
  const fallback = '/soundyear/media/dummy.svg';
...
function renderPlayersRanked(playersRanked) {
  ...
  list.forEach(p => {
    ...
    // NEU: Answer-Box links + CardWrap rechts (damit Card-Breite fix bleiben kann)
    row.innerHTML = `
...
function playQuestionAudio(audioFile) {
  ...
  // NEU: absolute URL (z.B. Deezer) direkt nutzen
  if (audioFile.startsWith('http://') || audioFile.startsWith('https://')) {
    questionAudio.src = audioFile;
  } else {
    questionAudio.src = `/soundyear/media/audio/${audioFile}`;
  }
...
socket.on('player_logged_in', data => {
  ...
  // NEU: Answer-Box sichtbar machen, aber noch leer lassen
...
socket.on('reveal_player_answers', data => {
  ...
  Object.entries(pa).forEach(([playerId, answerVal]) => {
    // 1) Player als "answered" markieren
    const row = getPlayerRow(playerId);
    if (row) row.classList.add('is-answered');

    // 2) Answer-Box einblenden + Text setzen (zentriert macht CSS)
    ...
    // Freitext: robust als String darstellen
    box.textContent = (answerVal === null || answerVal === undefined) ? '' : String(answerVal);
  });
});
...
socket.on('unveil_correct', data => {
  ...
  // FreetextStandardFlow sendet: { correct: ... }
  const y =
    (data && (data.correct ?? data.correct_year ?? data.correctYear ?? data.year ?? data.correct_answer ?? data.correctAnswer)) ?? null;

  showYearBox(y !== null ? String(y) : '—');
});
...
socket.on('show_resolution', data => {
  ...
  // ---------- Distance-Heatmap (wie im Controller), pro Spieler ----------
  // Prio 1: Backend liefert diff direkt: data.details[playerId].diff
  // Prio 2: sonst selber rechnen aus data.player_answers + data.correct
  ...
  // 1) Wenn details.diff vorhanden ist: direkt anwenden
  const details = (data && data.details) ? data.details : null;
  if (details) {
    Object.entries(details).forEach(([playerId, det]) => {
      const d = det && det.diff;
      const diffNum = Number(d);
      applyPlayerDistanceClass(playerId, Number.isFinite(diffNum) ? diffNum : null);
    });
    return;
  }

  // 2) Sonst: selbst rechnen aus player_answers + correct
  const pa = (data && data.player_answers) ? data.player_answers : null;
  const correctRaw =
    (data && (data.correct ?? data.correct_year ?? data.correctYear ?? data.year ?? data.correct_answer ?? data.correctAnswer)) ?? null;

  const corYear = extractYear(correctRaw);

  if (!pa || !Number.isFinite(corYear)) {
    // nichts zu tun
    return;
  }

  Object.entries(pa).forEach(([playerId, ansVal]) => {
    const myYear = extractYear(ansVal);
    if (Number.isFinite(myYear) && Number.isFinite(corYear)) {
      applyPlayerDistanceClass(playerId, Math.abs(myYear - corYear));
    } else {
      applyPlayerDistanceClass(playerId, null);
    }
  });
});
...
socket.on('play_round_video', data => {
  ...
  clearUnveil();
  clearAllScorePops();
  showVideo(`/soundyear/media/frage${data.round}.mp4`);
});
