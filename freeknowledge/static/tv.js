// --- FILE: ./freeknowledge/static/tv.js ---
...
function hideCorrectAnswerBox() {
  if (correctAnswerBox) correctAnswerBox.classList.add('is-hidden');
  if (correctAnswerValue) correctAnswerValue.textContent = '—';
}

function showCorrectAnswerBox(text) {
  if (correctAnswerBox) correctAnswerBox.classList.remove('is-hidden');
  if (correctAnswerValue) correctAnswerValue.textContent = (text ?? '—');
}
...
function setQuestionImage(imageFile) {
  const fallback = '/freeknowledge/media/dummy.svg';
...
function playQuestionAudio(audioFile) {
  ...
  // Wichtig:
  // - absolute URLs direkt nutzen
  // - absolute lokale Pfade mit / ebenfalls direkt nutzen
  // - nur reine Dateinamen auf /freeknowledge/media/audio/ prefixen
  questionAudio.src = isAbsolute ? file : `/freeknowledge/media/audio/${file}`;
...
socket.on('player_logged_in', data => {
  ...
  // Answer-Box sichtbar machen, aber noch leer lassen
...
socket.on('reveal_player_answers', data => {
  ...
  Object.entries(pa).forEach(([playerId, answerVal]) => {
    ...
    // 2) Answer-Box einblenden + Text setzen
...
socket.on('apply_scoring_update', data => {
  ...
  // Reihenfolge bewusst stabil, nur Scores aktualisieren
