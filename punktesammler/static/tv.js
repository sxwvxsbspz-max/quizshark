// --- FILE: ./punktesammler/static/tv.js ---
...
/* ---------- Game SFX ---------- */
const SFX_BASE = '/punktesammler/media/gamesounds';
...
function setQuestionImage(imageFile) {
  ...
  if (qImageEl) {
    qImageEl.src = isAbsolute ? file : `/punktesammler/media/images/${file}`;
    qImageEl.onerror = () => {
...
socket.on('play_round_video', data => {
  ...
  clearUnveil();
  clearAllScorePops();
  showVideo(`/punktesammler/media/frage${data.round}.mp4`);
});
