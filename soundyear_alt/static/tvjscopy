# --- FILE: ./engine/flows/freetext_standard.py ---
# (Basiert auf mc_standard.py – gleicher Flow/Timing/Reconnect/Pause, aber für Freitext-Antworten)
...
class FreetextStandardFlow:
    """
    Modul-agnostischer Standard-Flow (Freetext):
    - Timing/States/Reconnect/Pause identisch zu MCStandardFlow
    - Antworten sind Freitext (payload enthält z.B. "text")
    - Question-Schema ist freier (z.B. correct/answer_spec/metadata)
    - Scoring kann optional zusätzlich Details liefern (gained, details)
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Freitext-Antworten: je Spieler ein normalisierter Wert (z.B. str oder dict)
        ...
        # Optional: zusätzliche Details vom Scoring/Evaluator (z.B. correctness per player)
        ...
        # Sofortpause (nur an Safe-Points wirksam)
    ...
    def _question_payload(self) -> dict:
        """
        Vereinheitlicht, was bei show_question (TV+Controller) rausgeht.
        Erwartet im Question-Objekt mindestens "text".
        Optional:
          - audio, image
          - input_meta (z.B. placeholder, max_len, keyboard, hint)
          - prompt (alternativ zu text)
        """
        ...
        return {
            ...
            # Optional: Freitext-Metadaten für Frontend (wenn vorhanden)
    ...
    def _correct_payload(self) -> dict:
        """
        Vereinheitlicht, was bei unveil_correct / show_resolution rausgeht.
        Optional unterstützt:
          - correct: kanonische richtige Antwort (string/int/...)
          - answer_spec: strukturierte Spezifikation (range/regex/variants/...)
        """
        q = self.active_question or {}
        return {
    ...
    def sync_controller_state(self, sid: str):
        ...
        if self.state == "QUESTION_OPEN":
            ...
            # Reihenfolge: erst Spielerantworten, dann korrekt (wenn schon gesendet)
    ...
    def handle_event(self, player_id: str, action: str, payload: dict):
        ...
        if action == "submit_answer":
            ...
            # Freitext: normalize(payload) (AnswerType entscheidet, was akzeptiert wird)
            # Wir geben num_options bewusst NICHT vor, damit Text-AnswerTypes nichts davon abhängig machen.
            try:
                normalized = self.answer_type.normalize(payload)
            except TypeError:
                # Falls dein AnswerTypeBase noch die alte Signatur erwartet:
    ...
    def start_question_intro(self):
        ...
        # absolute Zeitpunkte für Frage & Unveil
    ...
    def _reveal_answers_with_optional_delay(self, token: int, reason: str):
        ...
        # 2) Danach: correct unveil + resolution
        self.core.start_task(
    ...
    def _unveil_correct_then_resolution(self, token: int):
        ...
        # Phase 2: korrekt "unveil" (Freitext: correct/spec statt correct_index)
    ...
    def _unpack_scoring_result(self, res: Any) -> Tuple[Dict[str, int], Optional[dict]]:
        """
        Unterstützt:
          - compute_gained(...) -> gained: Dict[str,int]
          - compute_gained(...) -> (gained: Dict[str,int], details: dict)
        """
