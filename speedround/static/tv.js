...
/* ---------- Background Music ---------- */
const bgm = new Audio('/speedround/media/gamesounds/background.mp3');
bgm.preload = 'auto';
bgm.loop = true;
bgm.volume = 1.0;
...
/* ---------- Game SFX ---------- */
const SFX_BASE = '/speedround/media/gamesounds';
...
function setQuestionImage(imageFile) {
  const fallback = '/speedround/media/dummy.svg';
  ...
  if (qImageEl) {
    qImageEl.src = isAbsolute ? file : `/speedround/media/images/${file}`;
    qImageEl.onerror = () => {
...
function playQuestionAudio(audioFile) {
  ...
  questionAudio.src = `/punktesammler/media/audio/${audioFile}`;
  questionAudio.load();
...
socket.on('play_round_video', data => {
  ...
  clearUnveil();
  clearAllScorePops();
  showVideo(`/speedround/media/frage${data.round}.mp4`);
});
