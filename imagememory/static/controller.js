// --- FILE: ./imagememory/static/controller.js ---
...
/* ---------- Memo Image ---------- */
const memoImageEl = document.getElementById("memo-image");

/* ---------- Timer-Bar ---------- */
const timebarFill     = document.getElementById("timebar-fill");
const memoTimebarFill = document.getElementById("memo-timebar-fill");
...
function setMemoImage(imageFile) {
  ...
  memoImageEl.src = file
    ? (isAbsolute ? file : `/imagememory/media/${file}`)
    : fallback;

  memoImageEl.onerror = () => {
    memoImageEl.onerror = null;
    memoImageEl.src = fallback;
  };
}
...
function stopTimebar() {
  ...
  if (memoTimebarFill) memoTimebarFill.style.width = "0%";
}


function startTimebarOn(fillEl, seconds, startedAt /* optional */) {
  if (!fillEl) return;
...
function startTimebar(seconds, startedAt /* optional */) {
  startTimebarOn(timebarFill, seconds, startedAt);
}

function startMemoTimebar(seconds, startedAt /* optional */) {
  startTimebarOn(memoTimebarFill, seconds, startedAt);
}
...
socket.on("show_memo", (data) => {
  ...
  // Timer so "justieren", dass remaining korrekt dargestellt wird (auch bei Reconnect/Delay)
  let startedAtForTimer = startedAt;
  if (startedAt && total > 0 && remaining >= 0 && remaining <= total) {
    const startedAtMs = parseStartedAtToMs(startedAt);
    if (startedAtMs !== null) {
      const elapsed = total - remaining;
      startedAtForTimer = Number(startedAtMs + elapsed * 1000);
    }
  }

  const dur = total > 0 ? total : (Number(data && data.duration) || 8);

  startMemoTimebar(dur, startedAtForTimer);
});
