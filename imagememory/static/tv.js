...
const timerWrap   = document.getElementById('question-timer');
const memoTimerWrap = document.getElementById('memo-timer');
...
/* ---------- Memo Audio (NEU) ---------- */
const memoAudio = new Audio();
memoAudio.preload = 'auto';
memoAudio.loop = true;
memoAudio.volume = 1.0;

let memoFadeRAF = null;
let memoFadeTO  = null;
let memoStopTO  = null;

function clearMemoFadeTimers() {
  if (memoFadeTO) { clearTimeout(memoFadeTO); memoFadeTO = null; }
  if (memoStopTO) { clearTimeout(memoStopTO); memoStopTO = null; }
  if (memoFadeRAF) { cancelAnimationFrame(memoFadeRAF); memoFadeRAF = null; }
}
...
function resolveMemoAudioSrc(v) {
  const file = (v || '').trim();
  if (!file) return '/imagememory/media/memoryimagedummy.mp3'; // Fallback
  ...
  // Wenn du später mal nur "foo.mp3" sendest:
  return isAbsolute ? file : `/imagememory/media/${file}`;
}

function fadeOutMemoAudio(durationMs) {
  clearMemoFadeTimers(); // nur RAF stoppen, nicht das Ende-StopTO (das setzen wir separat)
  const startVol = Number(memoAudio.volume ?? 1.0);
  const start = nowSyncedMs();
  const end = start + Math.max(0, Number(durationMs || 0));

  const tick = () => {
    const now = nowSyncedMs();
    const t = Math.min(1, Math.max(0, (now - start) / (end - start)));
    memoAudio.volume = Math.max(0, startVol * (1 - t));

    if (t < 1) memoFadeRAF = requestAnimationFrame(tick);
    else {
      memoFadeRAF = null;
      stopMemoAudio();
    }
  ...
  memoFadeRAF = requestAnimationFrame(tick);
}

function startMemoAudioWithFade(audioValue, startedAt, totalDurationSeconds) {
  stopMemoAudio(); // sicher sauber starten

  const src = resolveMemoAudioSrc(audioValue);
  memoAudio.src = src;
  memoAudio.load();
  memoAudio.volume = 1.0;
  ...
  // Fade-Out timing: 2s vor Ende
  const totalMs = Math.max(0, Number(totalDurationSeconds || 0) * 1000);
  const startedAtMs = parseStartedAtToMs(startedAt);

  // Wenn wir keine absolute Zeit haben: einfach relativ planen
  if (startedAtMs === null || totalMs <= 0) {
    // fallback: 8s memo -> fade ab 6s
    const endMs = Math.max(0, totalMs || 8000);
    const fadeStartDelay = Math.max(0, endMs - 2000);

    memoFadeTO = setTimeout(() => fadeOutMemoAudio(2000), fadeStartDelay);
    memoStopTO = setTimeout(() => stopMemoAudio(), endMs);
    return;
  }

  const endAtMs = startedAtMs + totalMs;
  const fadeAtMs = endAtMs - 2000;

  const delayToFade = fadeAtMs - nowSyncedMs();
  const delayToStop = endAtMs - nowSyncedMs();

  if (delayToFade <= 0) {
    // schon in der Fade-Phase
    fadeOutMemoAudio(2000);
  } else {
    memoFadeTO = setTimeout(() => fadeOutMemoAudio(2000), delayToFade);
  }

  if (delayToStop > 0) {
    memoStopTO = setTimeout(() => stopMemoAudio(), delayToStop);
  } else {
    stopMemoAudio();
  }
}
...
/* ---------- Background Music ---------- */
const bgm = new Audio('/imagememory/media/gamesounds/background.mp3');
bgm.preload = 'auto';
bgm.loop = true;
bgm.volume = 1.0;
...
/* ---------- Game SFX ---------- */
const SFX_BASE = '/imagememory/media/gamesounds';
...
if (memoLayer) memoLayer.style.display = 'none';
...
function showMemo() {
  ...
  if (gameLayer)  gameLayer.style.display  = 'none';
  if (memoLayer)  memoLayer.style.display  = 'block';
}
...
function setQuestionImage(imageFile) {
  // In ImageMemory gibt es NIE ein Bild pro Frage.
  // Deshalb immer die "unten"-Variante (Dummy/Logo-Box) anzeigen.
  const fallback = '/imagememory/media/dummy.svg';

  if (psRoot) psRoot.classList.remove('has-image');
...
function startTimerVisualOn(targetEl, seconds, startedAt /* optional */) {
  clearTimer();
  if (targetEl) targetEl.style.setProperty('--p', '0%');
  ...
  if (startedAtMs !== null) {
    ...
    const tick = () => {
      ...
      if (targetEl) targetEl.style.setProperty('--p', `${percent}%`);
...
function startTimerVisual(seconds, startedAt /* optional */) {
  startTimerVisualOn(timerWrap, seconds, startedAt);
}
...
function playQuestionAudio(audioFile) {
  ...
  questionAudio.src = `/imagememory/media/audio/${audioFile}`;
  questionAudio.load();
...
socket.on('play_round_video', data => {
  ...
  clearUnveil();
  clearAllScorePops();
  showVideo(`/imagememory/media/frage${data.round}.mp4`);
});
...
function setMemoImage(imageFile) {
  const img = document.getElementById('memo-image');
  if (!img) return;

  const fallback = '/imagememory/media/memoryimagedummy.png';
  ...
  img.src = file
    ? (isAbsolute ? file : `/imagememory/media/${file}`)
    : fallback;
...
function startMemoTimer(duration, startedAt) {
  startTimerVisualOn(memoTimerWrap, duration, startedAt);
}


function stopMemoTimer() {
  clearTimer();
  if (memoTimerWrap) memoTimerWrap.style.setProperty('--p', '0%');
}

socket.on('show_memo', data => {
  clearTimer();
  clearUnveil();
  showMemo();

  setMemoImage(data.image);

  const startedAt = data && (data.started_at ?? data.startedAt);
  const total = Number((data && (data.total_duration ?? data.totalDuration)) || 0);
  const remaining = Number((data && (data.remaining)) || 0);

  // Wenn Backend remaining+total liefert: Timer soll "als ob gestartet_at" laufen.
  // Wir rechnen startedAt so um, dass die Progress-Bar bei remaining korrekt steht.
  let startedAtForTimer = startedAt;
  if (startedAt && total > 0 && remaining >= 0 && remaining <= total) {
    const startedAtMs = parseStartedAtToMs(startedAt);
    if (startedAtMs !== null) {
      const elapsed = total - remaining;
      startedAtForTimer = startedAtMs + elapsed * 1000; // fake-start = "jetzt - remaining"
      startedAtForTimer = Number(startedAtForTimer);
    }
  }

  // Dauer für Visualisierung = total (fallback 8)
  const dur = total > 0 ? total : (Number(data && data.duration) || 8);

  startMemoTimer(dur, startedAtForTimer);
  startMemoAudioWithFade(data && data.audio, startedAt, dur);
