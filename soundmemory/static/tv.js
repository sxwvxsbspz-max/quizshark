...
const memoOverlay = document.querySelector('.ps__memoOverlay');
...
function emitMemoFinishedOnce() {
  socket.emit('module_event', { action: 'memo_finished' });
}
...
memoAudio.loop = false;
memoAudio.volume = 1.0;

function stopMemoAudio() {
  clearMemoFadeTimers();
  try {
    memoAudio.onended = null;
    memoAudio.pause();
    memoAudio.currentTime = 0;
    memoAudio.volume = 1.0;
  } catch (_) {}
    if (memoOverlay) memoOverlay.classList.remove('is-playing');
...
function resolveMemoAudioSrc(v) {
  const file = (v || '').trim();
  if (!file) return '/soundmemory/media/memoryimagedummy.mp3'; // Fallback
  ...
  return isAbsolute ? file : `/soundmemory/media/${file}`;
}


function startMemoAudioWithFade(audioValue, startedAt, totalDurationSeconds) {
  // totalDurationSeconds wird NICHT mehr zum Cutten benutzt (Frontend-getrieben).
  ...
  // Wenn Song endet -> Backend darf weitermachen
memoAudio.onended = () => {
  if (memoOverlay) memoOverlay.classList.remove('is-playing');
  emitMemoFinishedOnce();
};


    memoAudio.play()
    .then(() => {
      if (memoOverlay) memoOverlay.classList.add('is-playing');
    })
    .catch(() => {});
}
...
/* ---------- Background Music ---------- */
const bgm = new Audio('/soundmemory/media/gamesounds/background.mp3');
bgm.preload = 'auto';
bgm.loop = true;
bgm.volume = 1.0;
...
/* ---------- Game SFX ---------- */
const SFX_BASE = '/soundmemory/media/gamesounds';
...
function showGame() {
  ...
  // FIX: Timer standardmäßig aus (wird nur bei open_answers wieder an)
  if (timerWrap) timerWrap.classList.add('is-invisible');
}
...
function setQuestionImage(imageFile) {
  ...
  const fallback = '/soundmemory/media/dummy.svg';
...
function playQuestionAudio(audioFile) {
  ...
  questionAudio.src = `/soundmemory/media/audio/${audioFile}`;
  questionAudio.load();
...
socket.on('play_round_video', data => {
  ...
  clearUnveil();
  clearAllScorePops();
  showVideo(`/soundmemory/media/frage${data.round}.mp4`);
});
...
function setMemoImage(imageFile) {
  ...
  const fallback = '/soundmemory/media/memoryimagedummy.png';
  ...
  img.src = file
    ? (isAbsolute ? file : `/soundmemory/media/${file}`)
    : fallback;
...
socket.on('show_memo', data => {
  ...
  // Memo ist frontend-getrieben: Song-Ende triggert memo_finished
  startMemoAudioWithFade(data && data.audio);
});
