# --- FILE: ./soundmemory/logic.py ---
# (Fork von Punktesammler, erweitert um SoundMemory-Logik:
#  1 Bild + 1 Song pro "Runde", max_rounds = Fragen pro Bild)
...
# Flow
from engine.flows.mc_sound_memory import MCSoundMemoryFlow, MemoryQuizTiming
...
class ImageMemoryQuestionSource:
    """
    QuestionSource für SoundMemory (JETZT MIT LOGIK):
...
class SoundmemoryLogic:
    ...
    WICHTIG:
    - max_rounds = Anzahl Fragen pro Bild (1 Bild + 1 Song pro Runde!)
    - Memo ist FRONTEND-getrieben (Song-Ende triggert memo_finished)
    - Not-Aus nach 4 Minuten ist im Flow enthalten
    """
    ...
    def __init__(self, socketio, players, on_game_finished=None):
        ...
        # ---------------------------------------------
        # Timing (Memo: nur Anzeige; Not-Aus im Flow: 240s)
        # ---------------------------------------------
        timing = MemoryQuizTiming(
            intro_delay_seconds=3,
            memo_duration_seconds=0,   # Anzeige/Visual (nicht Master)
            memo_timeout_seconds=240,   # Not-Aus
