...
/* ---------- Background Music ---------- */
const bgm = new Audio('/soundtracks/media/gamesounds/background.mp3');
bgm.preload = 'auto';
bgm.loop = true;
bgm.volume = 1.0;
...
/* ---------- Game SFX ---------- */
const SFX_BASE = '/soundtracks/media/gamesounds';
...
function setQuestionImage(imageFile) {
  const fallback = '/soundtracks/media/dummy.svg';
  ...
  if (qImageEl) {
    qImageEl.src = isAbsolute ? file : `/soundtracks/media/images/${file}`;
    qImageEl.onerror = () => {
...
function playQuestionAudio(audioFile) {
  ...
  if (audioFile.startsWith('http://') || audioFile.startsWith('https://')) {
    ...
    questionAudio.src = `/soundtracks/media/audio/${audioFile}`;
  }
...
socket.on('play_round_video', data => {
  ...
  clearUnveil();
  clearAllScorePops();
  showVideo(`/soundtracks/media/frage${data.round}.mp4`);
});
