// --- FILE: ./vollereinsatz/static/controller.js ---
...
const wagerBoxEl    = document.getElementById("wager-box");
...
/* ---------- NEU: Controller Wager-Box ---------- */
let myWagerValue = null;

function clearWagerBoxStates() {
  if (!wagerBoxEl) return;
  wagerBoxEl.classList.remove("is-locked", "is-correct", "is-wrong");
}

function setWagerBoxState(state) {
  if (!wagerBoxEl) return;
  clearWagerBoxStates();
  if (state) wagerBoxEl.classList.add(state);
}

function resetWagerBox() {
  myWagerValue = null;
  if (!wagerBoxEl) return;
  wagerBoxEl.textContent = "";
  wagerBoxEl.classList.remove("is-set");
  wagerBoxEl.classList.add("is-empty");
  clearWagerBoxStates();
}

function setWagerBoxValue(value) {
  myWagerValue = Number(value);
  if (!wagerBoxEl) return;
  wagerBoxEl.textContent = String(myWagerValue);
  wagerBoxEl.classList.remove("is-empty");
  wagerBoxEl.classList.add("is-set");
  // State NICHT automatisch setzen: das machen die Events (locked/neutral/result)
}
...
function setQuestionImage(imageFile) {
  const fallback = '/vollereinsatz/media/dummy.png';
...
/* ---------- NEU: VollerEinsatz (WAGER) ---------- */
const DEFAULT_WAGER_VALUES = [25, 50, 100, 200];
let currentWagerValues = DEFAULT_WAGER_VALUES.slice();
let inputMode = "answer"; // "answer" | "wager"

function setOptionsText(values) {
  const arr = Array.isArray(values) ? values : [];
  btns.forEach((b, i) => {
    if (!b) return;
    b.textContent = (arr[i] !== undefined && arr[i] !== null) ? String(arr[i]) : "";
  });
}

function highlightMyWager(myWager) {
  resetAnswerVisuals();
  btns.forEach((b, i) => {
    if (!b) return;
    const v = currentWagerValues[i];
    if (Number(v) === Number(myWager)) {
      b.classList.add("is-selected");
      b.classList.remove("is-dim");
    } else {
      b.classList.add("is-dim");
...
socket.on("play_round_video", (data) => {
  inputMode = "answer";
  currentWagerValues = DEFAULT_WAGER_VALUES.slice();
...
socket.on("show_category", (data) => {
  ...
  if (qText) qText.textContent = (data && data.category) ? `Kategorie: ${String(data.category)}` : "Kategorie: ";

  setQuestionImage(null);

  // Keine Optionen in Category-Intro
  setOptionsText(["", "", "", ""]);

  // NEU: Header sofort setzen (Name + Score)
...
socket.on("show_wager", (data) => {
  ...
  currentWagerValues = Array.isArray(data && data.wager_values) && (data.wager_values.length >= 4)
    ? data.wager_values.slice(0, 4)
    : DEFAULT_WAGER_VALUES.slice();

  setOptionsText(currentWagerValues);
  ...
  clearScorePopInline();
    clearQuestionResult();
  resetWagerBox(); // leer/blass anzeigen solange noch nicht gesetzt
...
socket.on("unveil_wager", (data) => {
  ...
  const myWager = (myId && wagers && Object.prototype.hasOwnProperty.call(wagers, myId)) ? wagers[myId] : null;

  if (myWager !== null && myWager !== undefined) {
    showAnswers();
    highlightMyWager(myWager);

   // NEU: Wager-Box final aus Server-Snapshot setzen
setWagerBoxValue(myWager);
setWagerBoxState("is-locked");

  } else {
    // falls kein Snapshot da ist, einfach stehen lassen
    showAnswers();
    resetAnswerVisuals();
  }
...
/* ---------- optional: WAGER_UPDATE (kein UI-Element im Controller dafür) ---------- */
socket.on("wager_update", () => {
  // bewusst leer (TV zeigt "wer gesetzt hat"; Controller hat keine Player-Liste)
});
...
clearScorePopInline();
clearQuestionResult();
setWagerBoxState(null); // <- Rahmen weg/neutral, sobald die Frage kommt
...
// Wager-Box Rahmen = rot/grün wie Frage (für mich)
const myId2 = localStorage.getItem(LS_PLAYER_ID_KEY);
const myAnswer2 = (data.player_answers && myId2) ? data.player_answers[myId2] : null;

if (myAnswer2 !== null && myAnswer2 !== undefined) {
  if (Number(myAnswer2) === Number(correctIdx)) {
    setWagerBoxState("is-correct");
  } else {
    setWagerBoxState("is-wrong");
  }
}
...
btns.forEach(btn => {
  btn.addEventListener("pointerup", () => {
    ...
    if (inputMode === "wager") {
      const value = currentWagerValues[idx];
      if (value === undefined || value === null) return;

      lockAfterAnswer(idx);

   // NEU: sofort im UI anzeigen (ohne Animation)
setWagerBoxValue(value);
setWagerBoxState("is-locked");
