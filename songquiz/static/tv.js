...
/* ---------- Background Music ---------- */
const bgm = new Audio('/songquiz/media/gamesounds/background.mp3');
bgm.preload = 'auto';
bgm.loop = true;
bgm.volume = 1.0;
...
/* ---------- Game SFX ---------- */
const SFX_BASE = '/songquiz/media/gamesounds';
...
function setQuestionImage(imageFile) {
  const fallback = '/songquiz/media/dummy.svg';
  ...
  if (qImageEl) {
    qImageEl.src = isAbsolute ? file : `/songquiz/media/images/${file}`;
    qImageEl.onerror = () => {
...
function playQuestionAudio(audioFile) {
  ...
  if (audioFile.startsWith('http://') || audioFile.startsWith('https://')) {
    ...
    questionAudio.src = `/songquiz/media/audio/${audioFile}`;
  }
...
socket.on('play_round_video', data => {
  ...
  clearUnveil();
  clearAllScorePops();
  showVideo(`/songquiz/media/frage${data.round}.mp4`);
});
