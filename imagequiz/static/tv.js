...
/* ---------- Background Music ---------- */
const bgm = new Audio('/imagequiz/media/gamesounds/background.mp3');
bgm.preload = 'auto';
bgm.loop = true;
bgm.volume = 1.0;
...
/* ---------- Game SFX ---------- */
const SFX_BASE = '/imagequiz/media/gamesounds';
...
function setQuestionImage(imageFile) {
  const fallback = '/imagequiz/media/dummy.svg';
  ...
  if (qImageEl) {
    qImageEl.src = isAbsolute ? file : `/imagequiz/media/images/${file}`;
    qImageEl.onerror = () => {
...
function playQuestionAudio(audioFile) {
  ...
  questionAudio.src = `/imagequiz/media/audio/${audioFile}`;
  questionAudio.load();
...
socket.on('play_round_video', data => {
  ...
  clearUnveil();
  clearAllScorePops();
  showVideo(`/imagequiz/media/frage${data.round}.mp4`);
});
