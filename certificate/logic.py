...
class CertificateLogic:
    """
    Certificate-Modul mit 2 Phasen:
    ...
    def _emit_intro(self, room=None, to=None):
        ...
        if to is not None:
            self.socketio.emit("certificate_intro", payload, to=to)
        else:
            self.socketio.emit("certificate_intro", payload, room=room)

    def _emit_countdown(self, room=None, to=None):
        payload = self._countdown_payload()
        if to is not None:
            self.socketio.emit("certificate_countdown", payload, to=to)
        else:
            self.socketio.emit("certificate_countdown", payload, room=room)
