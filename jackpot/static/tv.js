...
/* ---------- Background Music ---------- */
const bgm = new Audio('/jackpot/media/gamesounds/background.mp3');
bgm.preload = 'auto';
bgm.loop = true;
bgm.volume = 1.0;
...
/* ---------- Game SFX ---------- */
const SFX_BASE = '/jackpot/media/gamesounds';
...
function setQuestionImage(imageFile) {
  const fallback = '/jackpot/media/dummy.svg';
  ...
  if (qImageEl) {
    qImageEl.src = isAbsolute ? file : `/jackpot/media/images/${file}`;
    qImageEl.onerror = () => {
...
function playQuestionAudio(audioFile) {
  ...
  questionAudio.src = `/jackpot/media/audio/${audioFile}`;
  questionAudio.load();
...
socket.on('play_round_video', data => {
  ...
  clearUnveil();
  clearAllScorePops();
  showVideo(`/jackpot/media/frage${data.round}.mp4`);
});
