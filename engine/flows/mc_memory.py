# --- FILE: ./engine/flows/mc_memory.py ---
# ImageMemory-Flow: basiert auf mc_standard, erweitert um einen MEMO-Screen
# vor jeder Frage (ein Bild pro Runde, Fragenanzahl = max_rounds).
...
class MemoryQuizTiming:
    ...
    # NEU: Memo-Screen Dauer (Bild zeigen, bevor die Frage kommt)
    memo_duration_seconds: float = 30.0
...
class MCMemoryFlow:
    ...
    MC-Flow mit zusätzlichem MEMO-Screen:
      INTRO_VIDEO -> MEMO (einmal) -> QUESTION_VIDEO -> QUESTION_INTRO -> QUESTION_OPEN -> REVEAL -> RESOLUTION -> SCORING -> ...

    Wichtig (wie gewünscht):
    ...
    Der QuestionSource muss pro Frage mindestens liefern:
      {
        ...
        "memo_audio": Optional[str],  # runden-bgm fürs Memo
        "memo_image": Optional[str],  # rundenbild fürs Memo
      }

    Dummy-Fallback (wie abgestimmt):
      - /imagememory/media/memoryimagedummy.png
      - /imagememory/media/memoryimagedummy.mp3
    """

    DUMMY_IMAGE = "/imagememory/media/memoryimagedummy.png"
    DUMMY_AUDIO = "/imagememory/media/memoryimagedummy.mp3"
    ...
    def sync_controller_state(self, sid: str):
        ...
        # Memo-Screen resync
        if self.state == "MEMO":
            remaining = float(self.timing.memo_duration_seconds)
            total = float(self.timing.memo_duration_seconds)
    ...
    def start_memo_once(self):
        ...
        - zeigt Memo
        - nach Memo: startet frage1.mp4 (play_next_video)
        """
        ...
        self._memo_duration = float(getattr(self.timing, "memo_duration_seconds", 0.0) or 0.0)
        ...
        # Nach Memo: erstes Frage-Video spielen
        self.core.start_task(self._after_memo_play_first_video, token)

    def _after_memo_play_first_video(self, token: int):
        seconds = float(getattr(self.timing, "memo_duration_seconds", 0.0) or 0.0)
        if seconds > 0:
