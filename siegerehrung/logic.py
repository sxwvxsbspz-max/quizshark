# --- FILE: ./siegerehrung/logic.py ---
from datetime import datetime, timezone


class SiegerehrungLogic:
    """
    Einfaches Siegerehrung-Modul:
    ...
    def get_players_ranked(self):
        ...
        for pid, p in self.players.items():
            items.append({
                ...
                # NEU: Finale-Survival als primärer Sort-Key (Fallback 0)
                "final_rounds_survived": int(p.get("final_rounds_survived", 0) or 0),
            })

        # NEU: Sort:
        # 1) final_rounds_survived desc
        # 2) score desc
        # 3) name asc
        # 4) id asc
        items.sort(
            key=lambda x: (
                -int(x.get("final_rounds_survived", 0) or 0),
        ...
        for i, it in enumerate(items):
            ...
            # Tie nur wenn sowohl rounds_survived als auch score gleich sind
            cur_key = (
                int(it.get("final_rounds_survived", 0) or 0),
                int(it.get("score", 0) or 0),
            )
    ...
    def _start_show(self):
        ...
        # direkt anzeigen
        self._emit_siegerehrung(room="tv_room", players_ranked=players_ranked)
        self._emit_siegerehrung(room="controller_room", players_ranked=players_ranked)

        # Sicherheitsnetz: falls nie ein "module_finished" kommt
        self.socketio.start_background_task(self._watchdog_finish_task)

    def _emit_siegerehrung(self, room=None, to=None, players_ranked=None):
        payload = {
        ...
        if to is not None:
            self.socketio.emit("show_siegerehrung", payload, to=to)
        else:
            self.socketio.emit("show_siegerehrung", payload, room=room)

    def sync_tv_state(self, sid):
        # bei Reload im Siegerehrung: einfach erneut das Board an den TV schicken
        if self.state == "SHOWING":
            self._emit_siegerehrung(to=sid)

    def sync_controller_state(self, sid):
        # bei Reload im Siegerehrung: einfach erneut das Board an den Controller schicken
        if self.state == "SHOWING":
            self._emit_siegerehrung(to=sid)
